...
const uri = "mongodb+srv://aitvikram:aitvikram@cluster0.uuzka.mongodb.net/gamezone?retryWrites=true&w=majority";
...
app.get("/",(req,res) => {
    ...
    //     const collection = client.db("gamezone").collection("posts");
    ...
    MongoClient.connect(uri, function(err, db) {
        ...
        dbo.collection(post_collec).find({}).toArray(function(err, results){
            ...
            if (err){
                console.log("Following is the rror");
                console.log(err);
                return;
            }else{
                console.log(results)
    ...
    // const client = new MongoClient(uri, { useNewUrlParser: true, useUnifiedTopology: true });
    // client.connect(err => {
    //   const collection = client.db(database_name).collection(post_collec);
...
app.post('/open_post', (req,res) =>{    
    ...
    client.connect(err => {
        ...
        collection.insertOne(myPost,function(err,res){
            if (err) throw err;
            console.log("Data inserted");
        })
        client.close();
